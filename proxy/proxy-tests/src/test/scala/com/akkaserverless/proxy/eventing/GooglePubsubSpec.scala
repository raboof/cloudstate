...
class GooglePubsubSpec extends WordSpec with BeforeAndAfterAll with Eventually with Matchers with ScalaFutures {
  ...
  var userFunction: AkkaServerlessRunner = _
  ...
  def startUserFunction(config: Config): AkkaServerlessRunner =
    new AkkaServerless()
  ...
  override protected def beforeAll(): Unit = if (runTheseTests) {
    ...
    userFunction = startUserFunction(
      ConfigFactory.parseString(s"""
        |akkaserverless.user-function-interface = "0.0.0.0"
        |akkaserverless.user-function-port = ${user_function_port}
        |akkaserverless.eventsourced.snapshot-every = 100
      """.stripMargin).withFallback(ConfigFactory.defaultReference()).resolve()
    )

    userFunctionDone = userFunction.run()

    node = AkkaServerlessProxyMain.start(
