...
package com.akkaserverless.proxy.stress
...
import com.akkaserverless.proxy.EntityDiscoveryManager
...
object CrdtStressSpec {
  def startNode(httpPort: Int, seedNode: Option[ActorSystem]): (ActorRef, ActorSystem) = {
    val config = ConfigFactory.parseString("""
        ...
        |akka.jvm-exit-on-fatal-error = false
      """.stripMargin).withFallback(ConfigFactory.defaultReference()).resolve()
    implicit val system = ActorSystem("akkaserverless-proxy", config)
    implicit val materializer = ActorMaterializer()

    val c = config.getConfig("akkaserverless.proxy")
