...
package com.akkaserverless.proxy.spanner

import akka.actor.typed.scaladsl.adapter.ClassicActorSystemOps
import akka.actor.{ActorSystem => ClassicSystem}
import akka.grpc.GrpcClientSettings
import com.akkaserverless.proxy.AkkaServerlessProxyMain
 ...
 * a [[SchemaCheck]] actor
 */
object AkkaServerlessSpannerProxyMain {

  final object Config {

    def fromTypesafeConfig(): Config =
      fromTypesafeConfig(ConfigFactory.load().getConfig("akkaserverless.proxy.spanner"))
  ...
  def main(args: Array[String]): Unit = {
    ...
    start(config)(AkkaServerlessProxyMain.start())
  }
