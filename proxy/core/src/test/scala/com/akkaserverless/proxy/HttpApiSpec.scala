...
import com.akkaserverless.proxy.PathTemplateParser.PathTemplateParseException
import com.akkaserverless.proxy.test._
  ...
  "HTTP API" must {
    ...
    "not allow selectors which do not exist as service methods" in {
      assertConfigurationFailure(
        ...
        "IllegalHttpConfig1",
        "HTTP API Config: Rule selector [wrongSelector] must be empty or [akkaserverless.proxy.test.IllegalHttpConfig1.fail]"
      )
    }
    ...
    "not allow path extractors which refer to repeated fields" in {
      assertConfigurationFailure(
        ...
        "IllegalHttpConfig3",
        "HTTP API Config: Repeated parameters [akkaserverless.proxy.test.IllegalHttpConfig3Message.illegal_repeated] are not allowed as path variables"
      )
    }
