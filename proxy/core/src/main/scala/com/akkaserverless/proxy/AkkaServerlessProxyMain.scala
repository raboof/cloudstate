...
package com.akkaserverless.proxy
...
import com.akkaserverless.proxy.telemetry.AkkaServerlessTelemetry
...
final class HealthCheckReady(system: ActorSystem) extends (() => Future[Boolean]) {
  ...
  private[this] final val timeoutMs =
    system.settings.config.getConfig("akkaserverless.proxy").getDuration("ready-timeout").toMillis.millis
...
object AkkaServerlessProxyMain {
  ...
  private def start(configuration: Option[Config]): ActorSystem = {
    ...
    implicit val system =
      configuration.fold(ActorSystem("akkaserverless-proxy"))(c => ActorSystem("akkaserverless-proxy", c))
    ...
    system.log.info(s"Starting AkkaServerless Proxy version [${BuildInfo.version}] running on [$jvmName $jvmVersion]")

    val c = system.settings.config.getConfig("akkaserverless.proxy")
    val serverConfig = new EntityDiscoveryManager.Configuration(c)
    val appConfig = new AkkaServerlessProxyMain.Configuration(c)
