...
import com.akkaserverless.eventing.{EventDestination => EventDestinationProto, EventSource => EventSourceProto}
...
trait EventDestination {
   ...
   * occurs, the stream should be terminated with an error. AkkaServerless will then handle retries, using exponential
