...
package com.akkaserverless.proxy.crdt

import akka.cluster.ClusterEvent.CurrentClusterState
import akka.cluster.ddata.{Key, ReplicatedDataSerialization, SelfUniqueAddress}
 ...
 */
@SerialVersionUID(1L)
final case class Vote private[crdt] (private[crdt] val state: Map[UniqueAddress, BigInt] = Map.empty,
  ...
  def result(decider: Vote.Decider, allowedVoterStatuses: Set[MemberStatus] = Set(MemberStatus.Up))(
      implicit currentClusterState: CurrentClusterState
  ): Boolean = {

    // Create a view so that when calculating the result we can break early once we've counted enough votes
    val votes: Iterable[Boolean] = currentClusterState.members.view
      .collect {
        case m if allowedVoterStatuses(m.status) => m.uniqueAddress
      }
      .map { key =>
        state.get(key).fold(false)(_.testBit(0))
      }
   ...
   */
  def getResult(decider: Vote.Decider,
