...
import com.akkaserverless.proxy.action.ActionProtocolSupportFactory
import com.akkaserverless.proxy.crdt.CrdtSupportFactory
import com.akkaserverless.proxy.eventing.{EventLogEventing, EventingManager, EventingSupport, ProjectionSupport}
import com.akkaserverless.proxy.eventsourced.EventSourcedSupportFactory
import com.akkaserverless.proxy.valueentity.EntitySupportFactory
...
class EntityDiscoveryManager(config: EntityDiscoveryManager.Configuration)(
  ...
  private[this] final def binding: Receive = {
    case sb: ServerBinding =>
      log.info(s"Akka Serverless proxy online at ${sb.localAddress}")
