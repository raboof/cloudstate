...
package com.akkaserverless.proxy.eventing
...
import com.akkaserverless.eventing.{
  Eventing,
  EventingProto,
  EventDestination => EventDestinationProto,
  EventSource => EventSourceProto
}
import com.akkaserverless.protocol.entity.{ClientAction, Metadata, MetadataEntry}
import com.akkaserverless.proxy.EntityDiscoveryManager.ServableEntity
import com.akkaserverless.proxy.UserFunctionRouter
import com.akkaserverless.proxy.entity.UserFunctionReply
import com.akkaserverless.proxy.protobuf.Options
...
object EventingManager {
  ...
  private def entityToCommand[Ref](consumer: EventConsumer): Flow[SourceEvent[Ref], MessageIn[Ref], NotUsed] =
    Flow[SourceEvent[Ref]].map { sourceEvent =>
      ...
      val maybeConsumerMethod =
        if (messageAny.typeUrl.startsWith("p.akkaserverless.com/") || messageAny.typeUrl.startsWith(
              "json.akkaserverless.com/"
            )) {
  ...
  def createDesintationEvent(payload: ProtobufAny, serviceName: String, maybeMetadata: Option[Metadata]) = {
    ...
    val (ceType, contentType, bytes) = payload.typeUrl match {
      case json if json.startsWith("json.akkaserverless.com/") =>
        (json.stripPrefix("json.akkaserverless.com/"), "application/json", decodeBytes(payload))
      case "p.akkaserverless.com/string" =>
        ("", "text/plain; charset=utf-8", decodeBytes(payload))
      case "p.akkaserverless.com/bytes" =>
  ...
  val ProtobufAnyMediaSubType = "vnd.akkaserverless.protobuf.any"
  ...
  private def encodeBytesToAny(bytes: Option[ByteString]): ProtobufAny =
    ProtobufAny("p.akkaserverless.com/bytes", encodeByteArray(bytes))

  private def encodeJsonToAny(bytes: Option[ByteString], jsonType: String): ProtobufAny =
    ProtobufAny("json.akkaserverless.com/" + jsonType, encodeByteArray(bytes))

  private def encodeUtf8StringBytesToAny(bytes: Option[ByteString]): ProtobufAny =
    ProtobufAny("p.akkaserverless.com/string", encodeByteArray(bytes))
  ...
  private def encodeStringToAny(string: String): ProtobufAny = {
    ...
    ProtobufAny("p.akkaserverless.com/string", builder.toByteString)
  }
