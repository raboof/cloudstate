...
package com.akkaserverless.tck

import com.akkaserverless.testkit.ServiceAddress
...
class TCK extends Suites({
  ...
  val combinations = config.getConfigList("akkaserverless-tck.combinations")
  val validNames = combinations.iterator.asScala.map(_.getString("name")).toSet
  val verify = config.getStringList("akkaserverless-tck.verify").asScala.toSet

  verify.filterNot(validNames) match {
    case x if !x.isEmpty =>
      throw new IllegalArgumentException("Configuration 'akkaserverless-tck.verify' contains non-existent names for combinations:" + x.mkString("[",",","]"))
    case _ => // All good
  }
  ...
  combinations.
    ...
    map(c => new ManagedAkkaServerlessTCK(TckConfiguration.fromConfig(c))).
    toVector
}: _*) with SequentialNestedSuiteExecution

object ManagedAkkaServerlessTCK {
...
class ManagedAkkaServerlessTCK(config: TckConfiguration) extends AkkaServerlessTCK("for " + config.name, ManagedAkkaServerlessTCK.settings(config)) {
  config.validate()
