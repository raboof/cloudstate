...
class ConfiguredAkkaServerlessTCK extends AkkaServerlessTCK(TCKSpec.Settings.loadFromConfig())

class AkkaServerlessTCK(description: String, val settings: TCKSpec.Settings)
    extends TCKSpec
    with ProxyTCK
    with ActionTCK
    with EntityTCK
  ...
  ("Akka Serverless TCK " + description) when {
