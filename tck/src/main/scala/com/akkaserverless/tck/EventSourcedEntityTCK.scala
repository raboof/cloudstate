...
import com.akkaserverless.protocol.entity.{EntityPassivationStrategy, TimeoutPassivationStrategy}
import com.akkaserverless.protocol.event_sourced.{EventSourced, EventSourcedStreamOut}
import com.akkaserverless.tck.model.eventsourced._
import com.akkaserverless.testkit.eventsourced.EventSourcedMessages._
...
trait EventSourcedEntityTCK extends TCKSpec {
  ...
  def verifyEventSourcedEntityProxy(): Unit = {
    ...
    "verify HTTP API" in eventSourcedTest { id =>
      ...
      client.http
        .request("akkaserverless.tck.model.EventSourcedTwo/Call", s"""{"id": "$id"}""")
