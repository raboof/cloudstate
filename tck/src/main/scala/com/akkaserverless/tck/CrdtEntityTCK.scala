...
import com.akkaserverless.tck.model.crdt._
import com.akkaserverless.testkit.crdt.CrdtMessages._
...
trait CrdtEntityTCK extends TCKSpec {
  ...
  def verifyCrdtEntityProxy(): Unit = {
    ...
    "verify HTTP API" in PNCounter.test { id =>
      ...
      client.http
        .request("akkaserverless.tck.model.crdt.CrdtTwo/Call", s"""{"id": "$id"}""")
        .futureValue mustBe "{}"
      interceptor
        .expectCrdtEntityConnection()
        .expectIncoming(init(ServiceTwo, id))
