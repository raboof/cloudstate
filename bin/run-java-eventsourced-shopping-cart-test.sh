...
if [ "$build" == true ] ; then
  ...
  sbt -Ddocker.username=akkaserverlessdev -Ddocker.tag=dev java-eventsourced-shopping-cart/docker:publishLocal
fi
...
spec:
  storeConfig:
    statefulStore:
      name: $statefulstore
  containers:
    - image: akkaserverless/java-eventsourced-shopping-cart:latest
